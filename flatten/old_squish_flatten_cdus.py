...
2. For each CDU, if it is a builder moves CDU: 
    2.1. Find the head EDU, creating a *replacements* dict that maps CDU ids to head EDU id
    2.2. For each EDU, if there is an outgoing link, add EDU id : head id to *replacements*
    2.3. For each EDU that is not the head, gather the text and squish into one block
    and assign to the head edu by adding to the dict *squish*, mapping head id : squish text
    2.4. Add all non-head EDU ids to the *redundant* list to be removed 
"""
import os 
import json 
import re
