...
corpus_path = current_folder + '/VAL_100_bert.json'
...
with open(corpus_path, 'r') as j:
    ...
    for game in games:
            ...
            #now want to separate by turn
            
            for line in text_list:
                if '<Arch' in line: 
                    nl = line.split('<Architect>')
                    turns.append(nl[0] + nl[1])
                else:
                    turns.append('\n')

    print_string = '\n'.join(turns)
    with open (current_folder+ '/intr_sample.txt', 'w') as txt_file:
